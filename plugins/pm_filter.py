...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('💠 ᴜᴘᴅᴀᴛᴇ 💠', url='https://t.me/PocketMoviesOfficial'),
            InlineKeyboardButton('💠 ᴍᴏᴠɪᴇs 💠', url='https://t.me/+I53oHG2-0YYzMGVl')
            ],[      
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b>Hey 👋🏻 {message.from_user.mention} 😍\n\n<i>🔖 Title : {search}\n📫 Your Files is Ready Now\n♻️ Join @PocketMoviesOfficial</i></b>"
    if imdb and imdb.get('poster'):
        try:
            Boss = await message.reply_photo(photo=imdb.get('poster'), caption=cap[:1024], reply_to_message_id, reply_markup=InlineKeyboardMarkup(btn))
            ...
            Boss1 = await message.reply_photo(photo=poster, caption=cap[:1024], reply_to_message_id, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(900)
            await Boss1.delete()
            await message.delete()
        except Exception as e:
            logger.exception(e)
            Boss2 = await message.reply_photo(photo="https://telegra.ph/file/82b5bbbab6d5e5593b6b2.jpg", caption=cap, reply_to_message_id, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(900)
            await Boss2.delete()
            await msg.delete()
    else:
        fuk = await message.reply_photo(photo="https://telegra.ph/file/8b42f6caf6ef5fd76766f.jpg", caption=cap, reply_to_message_id, reply_markup=InlineKeyboardMarkup(btn))
        await asyncio.sleep(900)
        await fuk.delete()
        await msg.delete()
    if spoll:
...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("I couldn't find any movie in that name tag admin.")
    ...
    if not movielist:
        k = await msg.reply("I couldn't find anything related to that. Check your spelling or Tag admin")
        await asyncio.sleep(30)
        await k.delete()
        return
